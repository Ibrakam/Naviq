...
import { GamificationSection } from '../gamification/GamificationSection';
import { ConstellationIcon } from '../common/constellation-icon';
import { translations, Language } from '../../i18n/translations';

interface DashboardProps {
  accessToken: string;
  user: any;
  onNavigate: (page: 'dashboard' | 'assessment' | 'assessment-results' | 'catalog' | 'course-catalog' | 'profile' | 'admin' | 'login') => void;
  onLogout: () => void;
}
...
export function Dashboard({ accessToken, user, onNavigate, onLogout }: DashboardProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f1529] via-[#1a2238] to-[#0f1529] relative overflow-hidden">
  ...
  return (
    <LanguageContext.Provider value={{ lang, setLang, t }}>
      <div className="min-h-screen text-white bg-gradient-to-br from-[#0f1529] via-[#1a2238] to-[#0f1529] relative overflow-hidden">
        ...
        {/* Header */}
        <header className="bg-white/10 backdrop-blur-xl border-b border-white/20 sticky top-0 z-50 shadow-lg relative">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl text-white" style={{ fontWeight: 700, letterSpacing: '-0.01em' }}>Naviq</span>
              </div>
            ...
            {!isMobile && (
              <nav className="flex items-center gap-4">
                <button
                  ...
                  className="flex items-center gap-2 text-white/70 hover:text-[#00E5A0] transition-colors font-medium"
                >
                ...
                {/* Language Switcher */}
                <div className="flex items-center gap-1 bg-white/10 rounded-lg p-1 ml-2">
                  {[
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <div className="absolute top-0 bottom-0 right-0 w-[78vw] max-w-sm bg-white/80 backdrop-blur-xl shadow-2xl rounded-l-3xl border-l border-white/10 transition-all duration-300 ease-out flex flex-col" style={{ right: "0" }}>
            <div className="px-6 py-6 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm text-white/60">{t.dashboard.signedInAs}</p>
                  <p className="text-base font-semibold text-white truncate max-w-[150px]">
                    {profile?.name || 'Student'}
                  </p>
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu">
                <Menu className="w-5 h-5 rotate-90 text-white/60" />
              </Button>
            </div>
            <nav className="px-4 py-6 space-y-3 flex-1 overflow-y-auto">
              {navItems.map((item) => (
                <button
                  ...
                  className="w-full flex items-center gap-4 rounded-2xl border border-white/10 bg-white/80 backdrop-blur-xl px-4 py-4 text-left shadow-sm hover:border-[#7B61FF]/30 hover:bg-[#7B61FF]/5 hover:text-[#7B61FF] transition-all"
                >
              ...
              <div className="pt-4 mt-4 border-t border-white/10">
                <p className="text-xs text-white/60 mb-2 px-2">{t.dashboard.language || 'Язык'}</p>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                        lang === language.code
                          ? 'bg-[#7B61FF] text-white'
                          : 'text-white/70 hover:text-[#7B61FF]'
                      }`}
                    >
                      {language.label}
                    </button>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10">
          ...
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl mb-2 text-white" style={{ fontWeight: 700 }}>
    ...
    <GamificationSection accessToken={accessToken} />
          ...
          {profile?.assessmentCompleted && (
            <Card
              className="group relative overflow-hidden p-0 mb-8 border-2 border-slate-200 shadow-2xl hover:shadow-2xl hover:shadow-[#7B61FF]/20 transition-all duration-300 rounded-xl"
              style={{
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  ...
                  <div className="flex-shrink-0">
                    <Button
                      ...
                      size="lg"
                      className="w-full md:w-auto bg-gradient-to-r from-[#7B61FF] to-[#5B9FFF] hover:from-[#6B51EF] hover:to-[#4B8FEF] text-white shadow-lg hover:shadow-xl transition-all px-6 py-6 md:py-3 text-base font-semibold rounded-lg"
                    >
          ...
          {!profile?.assessmentCompleted && (
            <Card
              className="p-6 mb-8 border-2 border-slate-200 rounded-xl shadow-2xl hover:shadow-[#7B61FF]/20 hover:border-[#7B61FF]/30 transition-all"
              style={{
                background: 'linear-gradient(135deg, #ffffff 0%, #faf9fb 100%)'
              }}
            >
          ...
          {profile?.assessmentCompleted && recommendedSimulations.length > 0 && (
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-white mb-4">{t.dashboard.recommendedForYou}</h2>
              <div className="grid md:grid-cols-3 gap-6">
                {recommendedSimulations.map((sim) => (
                  <Card
                    key={sim.id}
                    className="p-6 border-2 border-purple-200 rounded-xl shadow-2xl hover:shadow-purple-500/20 hover:border-purple-300 transition-all"
                    style={{
                  ...
                  >
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="px-2 py-1 bg-purple-100 rounded-md text-xs font-medium text-purple-700">
                        {sim.difficulty}
                      </span>
                    </div>
                    <h3 className="text-lg font-semibold text-slate-800 mb-2">{sim.title}</h3>
                    <p className="text-slate-600 text-sm mb-4 line-clamp-2">{sim.description}</p>
                    <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                      ...
                      <Button
                        size="sm"
                        onClick={() => onNavigate('catalog')}
                        className="bg-gradient-to-r from-[#7B61FF] to-[#5B9FFF] hover:from-[#6B51EF] hover:to-[#4B8FEF] text-white"
                      >
                        {t.dashboard.start}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-semibold text-slate-800">{t.dashboard.exploreSimulations}</h3>
              </div>
              <p className="text-slate-600 mb-4">
              ...
              <Button
                onClick={() => onNavigate('catalog')}
                className="bg-gradient-to-r from-[#7B61FF] to-[#5B9FFF] hover:from-[#6B51EF] hover:to-[#4B8FEF] text-white w-full"
              >
            ...
            <Card
              className="p-6 border-2 border-emerald-200 rounded-xl shadow-2xl hover:shadow-emerald-500/20 hover:border-emerald-300 transition-all"
              style={{
                background: 'linear-gradient(135deg, #ffffff 0%, #f0fdf4 100%)'
              }}
            >
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-gradient-to-br from-[#10B981]/10 to-[#059669]/10 rounded-xl flex items-center justify-center">
                  <Target className="w-5 h-5 text-[#10B981]" />
                </div>
                <h3 className="text-xl font-semibold text-slate-800">Обучающие курсы</h3>
              </div>
              <p className="text-slate-600 mb-4">
                Изучайте теорию и практику через структурированные уроки
              </p>
              <Button
                onClick={() => onNavigate('course-catalog')}
            ...
            <Card
              className="p-6 border-2 border-blue-200 rounded-xl shadow-2xl hover:shadow-blue-500/20 hover:border-blue-300 transition-all"
              style={{
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-semibold text-slate-800">{t.dashboard.yourProfile}</h3>
              </div>
              <p className="text-slate-600 mb-4">
