...
export function Profile({ accessToken, user, onBack, onNavigate }: ProfileProps) {
  ...
  const isProfileComplete = () => {
    ...
    return !!(
      ...
      (profile.education_status === 'Не учусь' || profile.school_name) &&
      (profile.education_status === 'Не учусь' || profile.graduation_date)
    );
  };

  const getMissingFields = () => {
    const missing: string[] = [];
    if (!profile?.name) missing.push('Имя');
    if (!profile?.date_of_birth) missing.push('Дата рождения');
    if (!profile?.education_status) missing.push('Статус обучения');
    if (profile?.education_status && profile.education_status !== 'Не учусь') {
      if (!profile?.school_name) missing.push('Название учебного заведения');
      if (!profile?.graduation_date) missing.push('Дата окончания обучения');
    }
    return missing;
  };
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 overflow-visible">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="text-3xl mb-2">{profile?.name || 'Пользователь'}</h1>
              <p className="text-gray-600 mb-4">{profile?.email}</p>
              <div className="flex items-center gap-4 text-sm text-gray-600">
                <span className="flex items-center gap-1">
                  <Calendar className="w-4 h-4" />
                  Присоединился {new Date(profile?.createdAt).toLocaleDateString('ru-RU')}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 overflow-visible">
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed">
                    Дополните информацию в профиле для лучшего опыта использования платформы
                  </p>
                </div>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 overflow-visible">
              <div className="space-y-6 overflow-visible">
                <div className="overflow-visible">
                  ...
                  <p className="text-gray-900 text-base">{profile?.name || 'Не указано'}</p>
                </div>
                <div className="overflow-visible">
                  <label className="text-sm font-medium text-gray-600 flex items-center gap-2 mb-2">
              ...
              <div className="space-y-6 overflow-visible">
                <div className="overflow-visible">
                  ...
                  <p className="text-gray-900 text-base">{profile?.education_status || 'Не указано'}</p>
                </div>
                {profile?.education_status && profile.education_status !== 'Не учусь' && (
                  <>
                    <div className="overflow-visible">
                      <label className="text-sm font-medium text-gray-600 flex items-center gap-2 mb-2">
                      ...
                      <p className="text-gray-900 text-base">{profile?.school_name || 'Не указано'}</p>
                    </div>
                    <div className="overflow-visible">
                      <label className="text-sm font-medium text-gray-600 flex items-center gap-2 mb-2">
                        <CalendarIcon className="w-4 h-4" />
                      ...
                      <p className="text-gray-900 text-base">
                        {profile?.graduation_date
                          ? new Date(profile.graduation_date).toLocaleDateString('ru-RU')
                          : 'Не указано'}
                      </p>
                    </div>
                  </>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <Card className="p-6 text-center">
            ...
            <p className="text-gray-600 text-sm">Пройденных стажировок</p>
          </Card>

          <Card className="p-6 text-center">
            <XCircle className="w-8 h-8 text-red-600 mx-auto mb-3" />
            <p className="text-3xl mb-1">{failedCount}</p>
            <p className="text-gray-600 text-sm">Проваленных попыток</p>
          </Card>

          <Card className="p-6 text-center">
            <Clock className="w-8 h-8 text-blue-600 mx-auto mb-3" />
            <p className="text-3xl mb-1">{totalHours}ч</p>
            <p className="text-gray-600 text-sm">Часов практики</p>
          </Card>

          <Card className="p-6 text-center">
            <Target className="w-8 h-8 text-purple-600 mx-auto mb-3" />
            <p className="text-3xl mb-1">{profile?.recommendedTracks?.length || 0}</p>
            <p className="text-gray-600 text-sm">Треков открыто</p>
          </Card>
        </div>
        ...
        {profile?.assessmentCompleted && profile?.assessmentResults && (
          <div className="mb-8">
            <h2 className="text-2xl mb-4">Твои карьерные треки</h2>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {index === 0 ? 'Основное направление' : 'Альтернатива'}
                      </p>
                    </div>
                  </div>
                </Card>
        ...
        <div className="mb-8">
          <h2 className="text-2xl mb-4">Сертификаты и результаты</h2>
          
          {simulationResults.length === 0 ? (
            <Card className="p-12 text-center">
              <Award className="w-16 h-16 text-gray-400 mx-auto mb-4" />
              <h3 className="text-xl mb-2">Пока нет завершённых стажировок</h3>
              <p className="text-gray-600">
                Начни проходить стажировки, чтобы получить сертификаты и опыт
              </p>
            </Card>
          ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="ml-15 space-y-2">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <span className="">
                              {result.score} / {result.maxScore} баллов ({result.percentage || Math.round((result.score / result.maxScore) * 100)}%)
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-sm">
                              {new Date(result.completedAt).toLocaleDateString('ru-RU')} в {new Date(result.completedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}
                            </span>
                          </div>

                          {allAttempts.length > 1 && (
                            <div className="flex items-center gap-4">
                              <span className="text-sm text-gray-600">Попыток:</span>
        ...
        {simulationResults.length > Object.keys(resultsBySimulation).length && (
          <div>
            <h2 className="text-2xl mb-4">История попыток</h2>
            <Card className="p-6">
